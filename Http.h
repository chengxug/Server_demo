...
public:
    ...
    std::string errorMessage() const { return err_msg; }
...
public:
    ...
    RadixRouteNode(const std::string& lbl = "")
        : label(lbl)
        , isEnd(false)
    {
    }
...
void RadixRouter::addRoute(const std::string& method, const std::string& uri, Handler handler)
{
    ...
    while (pos < uri.length())
    {
        ...
        if (current->children.find(nextChar) ==
            current->children.end())   // pos位置的字符不存在，从pos位置开始创建子节点
        {
